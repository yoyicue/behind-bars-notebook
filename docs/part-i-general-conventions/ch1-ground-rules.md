...
- 短杆默认长度为 2½ 间距，仅容纳一个尾巴（tail）或符杠（beam）。
...
- 单条符杠的厚度为 ½ 个谱间距（stave-space）。
- 多条符杠之间的间距为 ¼ 个谱间距。
- 所有音符杆（stems）都应穿过所有内符杠（inner beams）直达最外层符杠（outer beam），
  不应仅靠两端杆来承载符杠。

#### 分段符杠（Fractional beams）

- 用于处理非对称节奏结构，长度约为一个音符头宽度；
- 适用于三十二分音或更短音符中的独立节奏处理。

### 在谱内的符杠定位（Beam placing within the stave）

#### 水平符杠：

- 不应置于谱线之间的空隙中；
- 符杠应靠近或直接附着于谱线之上或之下；
- 如果放在空白区域，会导致符杠与谱线混淆，影响阅读。

#### 倾斜符杠：

- 倾斜的符杠两端必须都附着在谱线上；
- 特别注意四条符杠时，应略微扩大符杠间距，以便每条线都能附着在谱线上；
- 若不能做到这一点，务必保证打印质量高，避免细白区域消失。


### 符杠位置对杆长的影响（Beam placing affecting stem length）

- 单、双符杠时，杆长应尽可能接近标准的 3½ 个谱间距；
- 若音符位于谱"间"（space）上，保持标准杆长；
- 若音符位于谱"线"（line）上，略短（如 3¼）以避免符杠误入空隙。

#### 三条以上符杠：

- 最外层符杠应远离音符头，以空出足够空间；
- 每增加一条符杠，杆长应相应增加；
- 符杠不能比标准16分音符符杠位置更靠近音符头。

### 加线音符与符杠（Ledger notes and beams）
...
### 符杠方向判断规则（Direction of beam angle）

- 两音音程决定符杠方向；
- 多音组时，取最外两音为参考；
- 若最外两音音高相同 → 符杠水平；
- 若中间音更接近符杠 → 符杠亦可水平（concave pattern）；
- 反之，convex pattern → 倾斜符杠。

### 多音组符杠情况（Stem direction on beamed groups）
